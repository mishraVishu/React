...
const Body = () => {
    ...
    const fetchData = async () => {
        try{
            const data = await fetch("https://thingproxy.freeboard.io/fetch/https://www.swiggy.com/dapi/restaurants/list/v5?lat=28.57590&lng=77.33450&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING");
    ...
    return res.length === 0 ? <Shimmer /> : (
        <div className="body">
            ...
            <div className="res-container">
                {
                    tempList.map(res => <ResturantCard data={res} key={res.info?.id} />)
                }
            </div>
        </div>
    )
